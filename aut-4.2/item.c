...
void closecon() {
  ...
  for (c = curcon; c; c = c->back)
    c->ref = 0;
  /* delay restoretomark until just before the end of newcon/newdef, for
   * fprintexp */
  stillopen = 1;
}
...
void newcon(char *id, exp type) {
  ...
  for (c = curcon; c; c = c->back)
    if (c->id == id) {
      error();
      (void)fprintf(stderr,
                    "duplicate variable identifier in context: \"%s\"\n", id);
    }
  c = ALLOC(con);
  c->kind = CON;
  c->id = id;
  ...
  c->implicit = a;
  savemem();
  if (!type || !check(type) || !checkdegree(type, 1, 2) ||
      (curcon && !curcon->type)) {
    c->type = 0;
    c->deg = 0;
    c->code = 0;
...
void newdef(char *id, exp body, exp type, int noexpand) {
  ...
  if (!type || !check(type) || !checkdegree(type, 1, 2) ||
    ...
    d->deg = degree(type);
    d->code = flavor(type);
    if (!body || !check(body) || !checktype(body, type) ||
        (noexpand && !FLAG('f')))
      d->body = 0;
    else
      d->body = body;
  ...
  if (e) {
    ...
    if (e->where == curpar) {
      ...
      (void)fprintf(
          stderr,
          "definition covers item with same name on same level: \"%s\"\n", id);
    }
  }
  savevalue(id, (char *)d);
  if (body)
